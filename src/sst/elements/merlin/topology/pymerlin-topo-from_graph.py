...
class topoFromGraph(Topology):
    ...
    def build(self, endpoint):
        ...
        for r in range(self.graph_num_vertices):
            ...
            topo = routers[r].setSubComponent(self.router.getTopologySlotName(),"merlin.singlerouter",0)
        ...
        #open csv file to store connectivity 
        with open(f'{self.csv_files_path}/CON.csv', mode='w') as csv_CON:
            ...
            for edge in edgelist:
                source=edge[0]
                dest=edge[1]
                csv_writer.writerow([source, dest, router_next_port[source]])
            ...
            assert(min(router_next_port)==self.graph_degree) #If it is a regular graph
            del edgelist
        ...
        with open(f'{self.csv_files_path}/RT.csv', mode='w') as csv_RT:
            ...
            pathdict=pickle.load(open(self.pathdict_file, 'rb'))
            pathdict.sort() # for the csv file to be more clean
            for (source, destination), paths in pathdict.items():
                for path in paths:
                    csv_writer.writerow([source, destination, ' '.join(path)])
